...
Целью проекта является построение модели машинного обучения для прогнозирования розничного товарооборота (РТО) магазинов «Пятёрочка» на месяц вперёд (ноябрь).

Такая модель имеет огромное экономическое значение для компании, так как позволяет избежать издержек производства, оптимизировать запасы и сократить потери от утилизации скоропортящихся продуктов
